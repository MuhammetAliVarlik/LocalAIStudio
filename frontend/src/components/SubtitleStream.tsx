...
export const SubtitleStream: React.FC<SubtitleStreamProps> = ({ text, isUser, isTyping }) => {
  ...
  // AUTO-TRUNCATE: Only show the last 250 characters to prevent screen filling
  const displayText = text.length > 250 ? "..." + text.slice(-250) : text;
  ...
  return (
    ...
    <div className="absolute bottom-32 left-0 right-0 flex justify-center z-40 pointer-events-none px-8">
        <AnimatePresence mode='wait'>
            <motion.div 
                key={isUser ? 'user-bubble' : 'ai-bubble'}
                initial={{ opacity: 0, y: 10, scale: 0.95 }}
                animate={{ opacity: 1, y: 0, scale: 1 }}
                exit={{ opacity: 0, y: -10, scale: 0.95 }}
                transition={{ duration: 0.3 }}
                className={`
                    backdrop-blur-xl border border-white/10 shadow-2xl rounded-2xl px-8 py-6 
                    max-w-3xl w-full
                    ${isUser ? 'bg-zinc-900/80 text-zinc-300' : 'bg-black/80 text-cyan-50'}
                `}
            >
                {/* Clamp text to 3 lines max */}
                <p className="text-lg md:text-xl font-mono leading-relaxed text-center line-clamp-3 overflow-hidden text-ellipsis">
                    {displayText}
                    {isTyping && !isUser && (

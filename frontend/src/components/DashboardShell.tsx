...
export const DashboardShell: React.FC = () => {
  ...
  // Memoize active agent to prevent flicker
  ...
  const COMPONENTS = {
    ...
    [AppMode.RESEARCH]: ResearchLab,
    [AppMode.LANGUAGE]: LanguageCenter,
    [AppMode.NOTES]: KnowledgeBase,
    [AppMode.HEALTH]: BioHub,
    [AppMode.COMMS]: CommunicationCenter,
    [AppMode.SYSTEM]: SystemControl,
    [AppMode.CALENDAR]: TemporalCalendar,
  ...
  const ActiveComponent = COMPONENTS[mode as keyof typeof COMPONENTS];
          ...
          <div className="p-4 border-t border-white/5 bg-black/40 backdrop-blur-xl">
              <div className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full bg-zinc-800 border border-white/10 overflow-hidden">
                      {user?.avatarUrl && <img src={user.avatarUrl} className="w-full h-full object-cover" alt="user" />}
                  </div>
      ...
      <div className="flex-1 relative h-full flex flex-col z-10 overflow-hidden bg-black/20">
          ...
          <div className={`absolute inset-0 flex flex-col transition-all duration-700 ${mode === AppMode.VOICE ? 'opacity-100 z-20 pointer-events-auto' : 'opacity-0 z-0 pointer-events-none'}`}>
               ...
               <div className="absolute inset-0 flex items-center justify-center z-0">
                   {/* OPTIMIZATION: Only render 3D if in Voice Mode */}
                   {mode === AppMode.VOICE && (
                       <VoiceAvatar 
                           state={avatarState} 
          ...
          {/* === ACTIVE APP CONTAINER (LAG FIX) === */}
          <div className="relative w-full h-full z-10 pointer-events-auto">
              {/* Only render ONE component active at a time */}
              {mode !== AppMode.VOICE && ActiveComponent ? (
                  mode === AppMode.BUILDER || mode === AppMode.AUTOMATION ? (
                      ...
                      <div className="w-full h-full glass-panel overflow-hidden bg-[#09090b] rounded-none md:rounded-l-2xl border-l border-white/10">
                          <ActiveComponent />
                      </div>
                  )
              ) : null}
